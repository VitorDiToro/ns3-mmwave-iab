...
namespace ns3
{
	...
	MmWavePhy::SetDlSfAllocInfo(SfAllocInfo sfAllocInfo)
	{
		...
		NS_LOG_LOGIC("MmWavePhy insert sfAllocInfo for frame " << (uint32_t)sfAllocInfo.m_sfnSf.m_frameNum << " subframe " << (uint16_t)sfAllocInfo.m_sfnSf.m_sfNum << " at time " << m_frameNum << " " << (uint16_t)m_sfNum);
