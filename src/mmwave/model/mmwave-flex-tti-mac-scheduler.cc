		...
		for (auto itUeInfo = ueInfo.begin(); itUeInfo != ueInfo.end(); ++itUeInfo)
		{
			if (itUeInfo->second.m_iab)
			{
				...
				if (totalSymbolsRequested > maxSymAvailableForIab)
				{
					...
					NS_LOG_DEBUG("New symbols request DL " << (uint32_t)itUeInfo->second.m_maxDlSymbols << " UL " << (uint32_t)itUeInfo->second.m_maxUlSymbols);
				}
			}
		}
		...
		do
		{
			...
			if (ueSchedInfo.m_dlSymbols > 0)
			{
				...
				do
				{
					...
					SlotAllocInfo slotInfo(slotIdx++, SlotAllocInfo::DL_slotAllocInfo, SlotAllocInfo::CTRL_DATA, SlotAllocInfo::DIGITAL, itUeInfo->first);
					slotInfo.m_dci = dci;
					NS_LOG_DEBUG(this << " UE " << dci.m_rnti << " gets DL slots " << (unsigned)dci.m_symStart << "-" << (unsigned)(dci.m_symStart + dci.m_numSym - 1) << " tbs " << dci.m_tbSize << " mcs " << (unsigned)dci.m_mcs << " harqId " << (unsigned)dci.m_harqProcess << " rv " << (unsigned)dci.m_rv << " in frame " << ret.m_sfnSf.m_frameNum << " subframe " << (unsigned)ret.m_sfnSf.m_sfNum);
			...
			if (ueSchedInfo.m_ulSymbols > 0)
			{
				...
				do
				{
					...
					slotInfo.m_dci = dci;
					NS_LOG_DEBUG(this << " UE" << dci.m_rnti << " gets UL slots " << (unsigned)dci.m_symStart << "-" << (unsigned)(dci.m_symStart + dci.m_numSym - 1) << " tbs " << dci.m_tbSize << " mcs " << (unsigned)dci.m_mcs << " harqId " << (unsigned)dci.m_harqProcess << " rv " << (unsigned)dci.m_rv << " in frame " << ulSfn.m_frameNum << " subframe " << (unsigned)ulSfn.m_sfNum);
