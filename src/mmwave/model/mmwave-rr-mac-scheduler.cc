...
namespace ns3
{
				...
				if (itCqi == m_ueUlCqi.end())
				{
					...
					for (uint32_t j = 0; j < (m_phyMacConfig->GetNumRb() * m_phyMacConfig->GetNumChunkPerRb()); j++)
					{
						...
						if (chunkInd == j)
						{
							...
							NS_LOG_INFO("UL CQI report for RNTI " << (*itMap).second.at(i) << " chunk " << i << " SINR " << sinr << " frame " << frameNum << " subframe " << subframeNum << " slot " << slotNum);
						}
						else
						{
											  ...
											  unsigned int slotNum)
	{
		...
		for (it = m_rlcBufferReq.begin(); it != m_rlcBufferReq.end(); it++)
		{
			...
			if ((((*it).m_rlcTransmissionQueueSize > 0) || ((*it).m_rlcRetransmissionQueueSize > 0) || ((*it).m_rlcStatusPduSize > 0)) && (itRnti == rntiAllocated.end())) // UE must not be allocated for HARQ retx
		...
		do
		{
			...
			{
				if (((*it).m_rlcTransmissionQueueSize > 0) || ((*it).m_rlcRetransmissionQueueSize > 0) || ((*it).m_rlcStatusPduSize > 0))
				{
			...
			uint32_t rbgMask = 0;
			uint16_t i = 0;
			NS_LOG_DEBUG("DL Allocation frame " << frameNum << " subframe " << sfNum << " slot " << slotNum + 1 << " "
												<< " RNTI " << schedInfo.m_rnti << " LCs " << (uint16_t)(*itLcRnti).second << " bytes " << tbSize << " mcs " << (uint16_t)newTbInfoElem.m_mcs << " harqId " << (uint16_t)newTbInfoElem.m_harqProcess);
			while (i < rbgPerTb)
			{
											  ...
											  unsigned int slotNum)
	{
		...
		do
		{
			...
			NS_LOG_DEBUG("UL Allocation frame " << frameNum << " subframe " << sfNum << " slot " << slotNum + 1 << " "
												<< " RNTI " << schedInfo.m_dci.m_rnti << " startPRB " << (uint32_t)newTbInfoElem.m_rbStart << " nPRB " << (uint32_t)newTbInfoElem.m_rbLen << " CQI " << cqi << " MCS " << (uint32_t)newTbInfoElem.m_mcs << " TBsize " << newTbInfoElem.m_tbSize << " harqId " << (uint16_t)harqId);
	...
	{
		uint16_t frameNum = params.m_sfnSf >> 16;
		uint8_t sfNum = (params.m_sfnSf & 0xff00) >> 8;
		NS_LOG_INFO("Scheduling frame " << (unsigned)frameNum << " subframe " << (unsigned)sfNum);

		MmWaveMacSchedSapUser::SchedConfigIndParameters ret;
		ret.m_sfn = params.m_sfnSf;
		//	std::map<uint16_t,SchedInfo>& schedInfoMap = ret.m_schedInfoMap;
