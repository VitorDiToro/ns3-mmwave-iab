...
void MmWave3gppPropagationLossModel::SetMinLoss(double minLoss)
{
	m_minLoss = minLoss;
}
double
...
void MmWave3gppPropagationLossModel::UpdateConditionMap(Ptr<MobilityModel> a, Ptr<MobilityModel> b, channelCondition cond) const
{
	m_channelConditionMap[std::make_pair(a, b)] = cond;
	m_channelConditionMap[std::make_pair(b, a)] = cond;
}

char MmWave3gppPropagationLossModel::GetChannelCondition(Ptr<MobilityModel> a, Ptr<MobilityModel> b)
{
	channelConditionMap_t::const_iterator it;
	it = m_channelConditionMap.find(std::make_pair(a, b));
	if (it == m_channelConditionMap.end())
...
void MmWave3gppPropagationLossModel::SetConfigurationParameters(Ptr<MmWavePhyMacCommon> ptrConfig)
{
