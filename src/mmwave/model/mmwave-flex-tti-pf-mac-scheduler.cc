...
namespace ns3
{
	...
	MmWaveFlexTtiPfMacScheduler::DoSchedDlRlcBufferReq(const struct MmWaveMacSchedSapProvider::SchedDlRlcBufferReqParameters &params)
	{
		...
		else
		{
			...
			{
				NS_LOG_DEBUG("params m_rlcTransmissionQueueSize " << params.m_rlcTransmissionQueueSize << " m_rlcRetransmissionQueueSize " << params.m_rlcRetransmissionQueueSize << " m_rlcStatusPduSize " << params.m_rlcStatusPduSize << " m_txPacketSizes " << params.m_txPacketSizes.size());
				...
				if (itCqi == m_ueUlCqi.end())
				{
					...
					for (unsigned j = 0; j < m_phyMacConfig->GetTotalNumChunk(); j++)
					{
						...
						if (chunkInd == j)
						{
							...
							NS_LOG_INFO("UL CQI report for RNTI " << itMap->second.m_rntiPerChunk.at(i) << " chunk " << i << " SINR " << params.m_ulCqi.m_sinr.at(i) << " frame " << frameNum << " subframe " << subframeNum << " startSym " << startSymIdx);
						}
						else
						{
	...
	MmWaveFlexTtiPfMacScheduler::UpdateBusySymbolsForIab(uint8_t sfNum, uint8_t symIdx, int symAvail)
	{
		...
		if (m_iabScheduler && !m_split)
		{
			...
			if (busyResources.m_valid)
			{
				...
				NS_LOG_DEBUG("This subframe has a DCI stored, with "
							 << " frame " << busyResources.m_sfnSf.m_frameNum << " subframe " << (uint16_t)busyResources.m_sfnSf.m_sfNum << PrintSubframeAllocationMask(busyResources.m_symAllocationMask));
		...
		else if (m_iabScheduler && m_split)
		{
			...
			if (busyResources.m_valid)
			{
				...
				NS_LOG_DEBUG("This subframe has a DCI stored, with "
							 << " frame " << busyResources.m_sfnSf.m_frameNum << " subframe " << (uint16_t)busyResources.m_sfnSf.m_sfNum << " " << PrintSubframeAllocationMask(busyResources.m_symAllocationMask) << " total number of busy symbols " << numBusySymbols << " symAvail " << symAvail);
			}
			else
			{
	...
	MmWaveFlexTtiPfMacScheduler::DoSchedTriggerReq(const struct MmWaveMacSchedSapProvider::SchedTriggerReqParameters &params)
	{
				...
				else if (m_dlHarqInfoList.at(i).m_harqStatus == DlHarqInfo::NACK)
				{
					...
					if (symAvail >= dciInfoReTx.m_numSym)
					{
						if (!CheckOverlapWithBusyResources(symIdx, dciInfoReTx.m_numSym)) // TODOIAB: check if it can be allocated later)
						{
							...
							NS_LOG_DEBUG("UE" << dciInfoReTx.m_rnti << " gets DL slots " << (unsigned)dciInfoReTx.m_symStart << "-" << (unsigned)(dciInfoReTx.m_symStart + dciInfoReTx.m_numSym - 1) << " tbs " << dciInfoReTx.m_tbSize << " harqId " << (unsigned)dciInfoReTx.m_harqProcess << " harqId " << (unsigned)dciInfoReTx.m_harqProcess << " rv " << (unsigned)dciInfoReTx.m_rv << " in frame " << ret.m_sfnSf.m_frameNum << " subframe " << (unsigned)ret.m_sfnSf.m_sfNum << " RETX");
				...
				else if (harqInfo.m_receptionStatus == UlHarqInfo::NotOk)
				{
					...
					if (symAvail >= dciInfoReTx.m_numSym)
					{
						if (!CheckOverlapWithBusyResources(symIdx, dciInfoReTx.m_numSym)) // TODOIAB: check if it can be allocated later
						{
							...
							slotInfo.m_dci = dciInfoReTx;
							NS_LOG_DEBUG("UE" << dciInfoReTx.m_rnti << " gets UL slots " << (unsigned)dciInfoReTx.m_symStart << "-" << (unsigned)(dciInfoReTx.m_symStart + dciInfoReTx.m_numSym - 1) << " tbs " << dciInfoReTx.m_tbSize << " harqId " << (unsigned)dciInfoReTx.m_harqProcess << " rv " << (unsigned)dciInfoReTx.m_rv << " in frame " << ulSfn.m_frameNum << " subframe " << (unsigned)ulSfn.m_sfNum << " RETX");
							ret.m_sfAllocInfo.m_slotAllocInfo.push_back(slotInfo);
							ret.m_sfAllocInfo.m_numSymAlloc += dciInfoReTx.m_numSym;
		...
		for (auto ueInfo = m_ueSchedInfoMap.begin(); ueInfo != m_ueSchedInfoMap.end(); ++ueInfo)
		{
			...
			if (ueInfo->second.m_iab && (ueInfo->second.m_totBufDl > 0 || ueInfo->second.m_totBufUl > 0))
			{
				...
				NS_LOG_DEBUG("IAB " << ueInfo->second.m_rnti << " request DL " << totalDlBufferRequested << " UL " << totalUlBufferRequested << " max DL symbols granted " << maxDlSym << " max UL symbols granted " << maxUlSym << " max tot " << maxDlSym + maxUlSym << " maxSymAvailableForSingleIab " << maxSymAvailableForSingleIab);
		...
		for (itUeAllocMap = ueAllocMap.begin(); itUeAllocMap != ueAllocMap.end(); itUeAllocMap++)
		{
			...
			if (ueInfo->m_dlSymbols > 0)
			{
				...
				do
				{
					...
					slotInfo.m_dci = dci;
					NS_LOG_DEBUG("UE" << dci.m_rnti << " gets DL symbols " << (unsigned)dci.m_symStart << "-" << (unsigned)(dci.m_symStart + dci.m_numSym - 1) << " tbs " << dci.m_tbSize << " mcs " << (unsigned)dci.m_mcs << " harqId " << (unsigned)dci.m_harqProcess << " rv " << (unsigned)dci.m_rv << " in frame " << ret.m_sfnSf.m_frameNum << " subframe " << (unsigned)ret.m_sfnSf.m_sfNum);
		...
		for (itUeAllocMap = ueAllocMap.begin(); itUeAllocMap != ueAllocMap.end(); itUeAllocMap++)
		{
			...
			if (ueInfo->m_ulSymbols > 0)
			{
				...
				do
				{
					...
					slotInfo.m_dci = dci;
					NS_LOG_DEBUG("UE" << dci.m_rnti << " gets UL symbols " << (unsigned)dci.m_symStart << "-" << (unsigned)(dci.m_symStart + dci.m_numSym - 1) << " tbs " << dci.m_tbSize << " mcs " << (unsigned)dci.m_mcs << " harqId " << (unsigned)dci.m_harqProcess << " rv " << (unsigned)dci.m_rv << " in frame " << ret.m_sfnSf.m_frameNum << " subframe " << (unsigned)ret.m_sfnSf.m_sfNum);
