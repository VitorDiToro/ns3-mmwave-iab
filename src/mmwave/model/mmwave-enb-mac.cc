...
namespace ns3
{
	...
	MmWaveEnbMac::MmWaveEnbMac(void) : m_frameNum(0),
									   m_sfNum(0),
									   m_slotNum(0),
									   m_tbUid(0)
	{
	...
	MmWaveEnbMac::DoReceivePhyPdu(Ptr<Packet> p)
	{
		...
		NS_ASSERT_MSG(rntiIt != m_rlcAttached.end(), "could not find RNTI " << rnti << " in m_cellId " << m_cellId);
			...
			if (slotAllocInfo.m_slotType != SlotAllocInfo::CTRL && slotAllocInfo.m_tddMode == SlotAllocInfo::DL_slotAllocInfo)
			{
				...
				else
				{
					...
					if (dciElem.m_ndi == 1)
					{
						...
						NS_ASSERT_MSG(rlcPduInfo.size() > 0, "rlcPduInfo.size () " << rlcPduInfo.size() << " in cell " << m_cellId << " dci for frame " << ind.m_sfnSf.m_frameNum << " subframe " << (uint16_t)ind.m_sfnSf.m_sfNum << " symbol " << (uint16_t)slotAllocInfo.m_dci.m_symStart << " rnti " << rnti);
	...
	MmWaveEnbMac::DoAddUe(uint16_t rnti)
	{
		...
		std::pair<std::map<uint16_t, std::map<uint8_t, LteMacSapUser *>>::iterator, bool>
			ret = m_rlcAttached.insert(std::pair<uint16_t, std::map<uint8_t, LteMacSapUser *>>(rnti, empty));
