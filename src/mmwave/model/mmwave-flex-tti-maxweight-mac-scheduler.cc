...
namespace ns3
{
	...
	void
	MmWaveFlexTtiMaxWeightMacScheduler::SetIabScheduler(bool iabScheduler)
	{
	...
	{
		NS_FATAL_ERROR("Add support for this method");
		m_iabBackahulSapProvider = sap;
	}

	MmWaveUeMacCschedSapProvider *
	MmWaveFlexTtiMaxWeightMacScheduler::GetMmWaveUeMacCschedSapProvider()
	{
		NS_FATAL_ERROR("Add support for this method");
				...
				while (!flowFound && flowIt != m_flowHeap.end() && symAvail > 0) // find flow with CQI in range (not in outage)
				{
					...
					if (!flow->m_isUplink && symAvail > 0)
					{
						...
						if (cqi != 0)
						{
							...
							NS_LOG_DEBUG("UE" << ueInfo->m_rnti << " LCID " << (unsigned)flow->m_lcid << " assigned " << (unsigned)ueInfo->m_dlSymbols << " DL symbols at MCS " << (unsigned)ueInfo->m_dlMcs << " (remaining == " << symAvail << ")");
					...
					else if (flow->m_isUplink && symAvail > 0)
					{
						...
						if (cqi != 0)
						{
							...
							NS_LOG_DEBUG("UE" << ueInfo->m_rnti << " LCID " << (unsigned)flow->m_lcid << " assigned " << (unsigned)ueInfo->m_ulSymbols << " UL symbols at MCS " << (unsigned)ueInfo->m_ulMcs << " (remaining == " << symAvail << ")");
							uint32_t sduSize = pduSize - (m_rlcHdrSize + m_subHdrSize);
							// flow->m_totalSchedSize += sduSize;
							flow->m_totalBufSize -= sduSize;
							flow->m_schedPacketSizes.push_front(sduSize);
