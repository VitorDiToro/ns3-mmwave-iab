...
namespace ns3
{
													  ...
													  Ptr<const MobilityModel> b) const
	{
		...
		*/
		...
		double relativeSpeed = (rxSpeed.x - txSpeed.x) + (rxSpeed.y - txSpeed.y) + (rxSpeed.z - txSpeed.z);
	...
	*/
	...
	MmWaveChannelMatrix::GenSinglePath(double hAngle, double vAngle, uint8_t *antennaNum) const
	{
		...
		for (int vIndex = 0; vIndex < vSize; vIndex++)
		{
			...
			{
				double w = (-2) * M_PI * hIndex * m_antennaSeparation * cos(hAngle) + (-2) * M_PI * vIndex * m_antennaSeparation * cos(vAngle);
