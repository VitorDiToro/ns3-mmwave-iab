...
namespace ns3
{
	...
	struct MacPduInfo
	{
		MacPduInfo(SfnSf sfn, uint32_t size, uint8_t numRlcPdu) : m_sfnSf(sfn), m_size(size), m_numRlcPdu(numRlcPdu)
		{
			m_pdu = Create<Packet>();
		...
		MacPduInfo(SfnSf sfn, uint32_t size, uint8_t numRlcPdu, DciInfoElementTdma dci) : m_sfnSf(sfn), m_size(size), m_numRlcPdu(numRlcPdu)
		{
			m_pdu = Create<Packet>();
			m_macHeader = MmWaveMacPduHeader();
